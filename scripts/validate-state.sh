...
try:
    ...
    print("FAIL: PyYAML is required (python module 'yaml' not found)")
    sys.exit(2)
...
def check_datetime(value, path):
    ...
    try:
        ...
        if dt.tzinfo is None:
            errors.append(f"FAIL: Field '{path}' must include timezone (e.g. Z)")
    except Exception:
...
if "slice_board" in data and isinstance(data["slice_board"], list):
    ...
    if total > 0:
        ...
        if abs(expected_pct - done_pct) > 30:
            print(
                f"WARN: overall_completion ({expected_pct}%) seems inconsistent with slice board ({done_count}/{total} done ≈ {done_pct}%)"
            )
